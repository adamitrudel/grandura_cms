class Page < ActiveRecord::Base
  ...
  def body_content
    PagePart.find(:first, :conditions => {:name => "body", :page_id => self.id}).content
  end
  
  def skip?
    !!(PagePart.find(:first, :conditions => {:name => "skip", :page_id => self.id}))
  end
  
  def first_position_child
    Page.find(:first, :conditions => { :parent_id => self.id, :position => 1 })
  end
  
  def skip_to
    x = self
    while (x.skip? && y = x.first_position_child)
      x = y
    end
    x
  end
